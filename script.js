$('.input-field input, .input-field select').on('change', function() {
    ...
    if (sex && age && height && weight && brac) {
        ...
        document.getElementById('result').innerHTML = `Alcohol (40%) dose in grams: ${alcohol40DoseGram.toFixed(2)}<br>Mixer dose: ${mixerDose.toFixed(2)} grams<br>Total drink: ${totalDrink.toFixed(2)} grams`;
    } else {
